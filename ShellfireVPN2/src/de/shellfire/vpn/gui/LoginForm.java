...
import de.shellfire.vpn.CryptFactory;
...
public class LoginForm extends javax.swing.JFrame {
   ...
   */
	...
	private void restoreAutoStartFromRegistry() throws RemoteException {
		boolean autoStart = Connection.vpnAutoStartEnabled();
	...
	class LoginTask extends SwingWorker<WsLoginResult, Object> {
		/*
		 ...
		 */
		public void done() {
			WsLoginResult loginResult = null;
			try {
				loginResult = get();
          ...
          try {
            if (jAutoStart.isSelected()) {
              ...
              Connection.removeVpnFromAutoStart();
            }
            ...
            Util.handleException(e);
					...
					VpnSelectDialog dia = new VpnSelectDialog(currentForm,
							service, jAutoConnect.isSelected());
		...
		@Override
		protected WsLoginResult doInBackground() throws Exception {
			String user = getUser();
			String password = getPassword();
			System.out.println("service.login() - start()");
			WsLoginResult loginResult = service.login(user, password);
