    ...
    if (internetAvailable) {
      ...
      if (updater.newVersionAvailable()) {
        ...
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION,
            i18n.tr("A new version of Shellfire VPN is available. An update is mandatory. Would you like to update now?"), ButtonType.YES,
            ButtonType.NO);
        ...
        if ((result.isPresent()) && (result.get() == ButtonType.YES)) {
          ...
          Process p;
          try {
            p = new ProcessBuilder(cmds).directory(new File(com.apple.eio.FileManager.getPathToApplicationBundle() + "/Contents/Java/"))
                .start();
